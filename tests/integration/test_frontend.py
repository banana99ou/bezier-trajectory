...
def test_loiter_returns_the_occlusion_layers(loiter):
    ...
    **`shadow_balls` is deliberately empty since the center-surface rewrite.** It
    carried the conservative containing ball the old occlusion builder wrapped the
    occluder in, and the page drew a static cone from it. No such ball exists any
    more: the shadow is generated by the centreline's own shadow surface and its
...
def test_loiter_occlusion_normals_carry_a_time_component(loiter):
    ...
    **This assertion is the reverse of the one it replaces, and the reversal is
    the point.** Until the center-surface rewrite a shadow half-space was a prism
    with time-parallel walls, so its normal had an exactly zero time coefficient
    "by design" -- and that made section 2.3's claim (the normal's time component
    is never zero) false of section 2.4's rows. The shadow is now generated by the
    centreline's own shadow surface, which carries the obstacle's time coordinate,
