...
- **`doc/`** — design record, notation lock, paper drafts (`papers/`, repo root), generated result tables. **Read `doc/design_freeze.md` and `doc/notation.md` before touching solver code or the paper.**
...
Sweeps KOZ altitudes at N=7 across all segment counts to isolate regimes where subdivision count actually matters.
...
- **`doc/design_freeze.md`** — design of record: formulation, every parameter's selection principle, evidence log. Outranks the paper.
- **`doc/notation.md`** — the single source of truth for mathematical symbols (`python3 tools/check_notation.py`).
- **`papers/paper_draft_korean_rev2.md`** — current manuscript; `papers/paper_draft_korean_rev2_PF.md` carries the professor's handwritten feedback transcribed inline.
- **`doc/results/paper_tables.md`** — the paper's result tables, generated by `tools/build_tables.py`.
- **`doc/session_handoff.md`** — in-flight work and context that is otherwise lost.
