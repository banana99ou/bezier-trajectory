...
SCENARIO_ELASTIC_WEIGHT = {
    ...
    # ladder certified at 1e5 and no lower rung. The 2026-08-28 rescale to
    # metres and seconds moved it up one decade, because the exact-penalty
    # threshold scales with the problem: at 1e5 the same run now returns an
    # occlusion certificate of 4.2e-05 -- a trajectory standing on slack, which
    # the figure gate refuses -- and at 1e6 it returns 0.0.
    "loiter": 1000000.0,
}
