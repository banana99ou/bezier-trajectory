...
def compute_profile_ylims(results, segcounts):
    ...
    for seg_count, P_opt, info in results:
        ...
        # Convert tau-derivatives to physical time derivatives using the transfer
        # time THIS SOLVE ACTUALLY USED.
        #   v(t) = (1/T) r'(tau)
        #   a_geom(t) = (1/T^2) r''(tau)
        # Read it from the solver rather than the module constant: T enters as 1/T
        # and 1/T^2, so a non-default transfer time silently rescales these curves
        # (a 20% change in T moves the acceleration by 1.44x) while the title,
        # which goes through control_effort_metrics, stays correct.
        T = float(info.get('T_transfer_s', TRANSFER_TIME_S))
...
def create_acceleration_figure(
    ...
    positions = np.array([curve.point(t) for t in ts])
    # The transfer time this solve used, not the module default — see the note in
    # compute_profile_ylims. Using the constant here while the panel title reads
    # info['T_transfer_s'] put two different T values on one set of axes.
    T = float(info.get('T_transfer_s', TRANSFER_TIME_S))
