...
class TransferBenchmark:
    ...
    def run_collocation(
        ...
        l1_lambda : float  ℓ₁ 정규화 강도 (기본 0)
        """
        solver = CollocationSolver(x_init=x_init, u_init=u_init, t_init=t_init,
                                   l1_lambda=l1_lambda)
        result = solver.solve(self.config)
        self._results["collocation"] = result
        return result

    def run_blade(self, **blade_kwargs) -> BenchmarkResult:
        """BLADE-SCP 최적화 실행.

        Parameters
        ----------
        **blade_kwargs
            BladeSolver 파라미터. 예: K=12, n=2, l1_lambda=0.1,
            t_f_override=4500.0 (콜로케이션 T_f에 맞출 때)
        """
        from .solvers import BladeSolver
        solver = BladeSolver(**blade_kwargs)
        result = solver.solve(self.config)
        self._results["blade"] = result
        return result

    def run_blade_collocation(self, **kwargs) -> BenchmarkResult:
        """BLADE-Collocation 순차 최적화 실행.

        Parameters
        ----------
        **kwargs
            BladeCollocationSolver 파라미터.
            예: blade_K=12, blade_l1_lambda=0.1, coast_threshold=0.05
        """
        from .solvers import BladeCollocationSolver
        solver = BladeCollocationSolver(**kwargs)
        result = solver.solve(self.config)
        self._results["blade_collocation"] = result
        return result
    ...
    def run_all(
        ...
        include_blade: bool = False,
        include_blade_collocation: bool = False,
        lambert_tof: float | None = None,
        blade_kwargs: dict | None = None,
        blade_collocation_kwargs: dict | None = None,
        ...
        blade_kwargs : dict, optional  BladeSolver 생성자 인수
        blade_collocation_kwargs : dict, optional  BladeCollocationSolver 생성자 인수
        ...
        if include_blade:
            self.run_blade(**(blade_kwargs or {}))
        if include_blade_collocation:
            self.run_blade_collocation(**(blade_collocation_kwargs or {}))
