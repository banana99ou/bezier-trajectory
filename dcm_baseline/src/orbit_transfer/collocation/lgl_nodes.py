...
def compute_lgl_nodes(N):
    ...
    # 짝수 N(=홀수 노드 수)일 때만 내부 노드 N-1개 중 가운데가 정확히 0 (P'_N이 기함수).
    # 홀수 N일 때는 0인 내부 노드가 없으며, 위 대칭 루프가 이미 N-1개의 내부근을 모두
    # 채운다. 과거 코드는 홀수 N에서도 가운데를 0으로 강제하여(아래 두 줄의 옛 버그)
    # 실제 근을 0으로 덮어써 P_N(0)=0에 의한 0-나눗셈으로 가중치/미분행렬을 비유한값으로
    # 망가뜨렸다. 짝수 N의 진짜 가운데 노드만 0으로 설정한다.
    if N % 2 == 0:
        tau[N // 2] = 0.0
