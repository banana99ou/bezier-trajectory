...
class HermiteSimpsonCollocation:
    ...
    def solve(self, x_guess=None, u_guess=None, nu0_guess=None, nuf_guess=None,
        ...
        # 비용함수 (Simpson 적분)
        # L2: ∫‖u‖²dt,  L1: λ₁∫‖u‖dt  (smooth: sqrt(u'u + ε²))
        eps_l1 = 1e-8  # L1 smoothing parameter
        J = 0
        for k in range(self.M):
            ...
            # L1 항 (코스팅 유도)
            if self.l1_lambda > 0:
                norm_k = ca.sqrt(ca.dot(u_k, u_k) + eps_l1)
                norm_m = ca.sqrt(ca.dot(u_m, u_m) + eps_l1)
                norm_k1 = ca.sqrt(ca.dot(u_k1, u_k1) + eps_l1)
                J += self.l1_lambda * (h / 6.0) * (
                    norm_k + 4 * norm_m + norm_k1
                )
        opti.minimize(J)
