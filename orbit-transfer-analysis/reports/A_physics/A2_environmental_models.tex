...
\slimtitle{A2. 환경 모델: J2 섭동 및 대기 항력}

\begin{document}
\drawborder
\thispagestyle{firstpage}

\lightertitle{A2. 환경 모델: J2 섭동 및 대기 항력}
...
\begin{abstract}
저궤도(LEO) 연속 추력 궤도전이 최적화에서 사용하는 환경 모델을 정의한다. 본 프로젝트의 동역학 모델은 다음 세 가지 가속도 성분의 합으로 구성된다.
\[
...
\subsection{지구 중력 포텐셜과 구면조화함수 전개}
...
여기서:
\begin{itemize}
  \item $R_E = 6378.137\;\text{km}$: 지구 적도 반지름
  \item $J_n$: $n$차 대영(zonal) 조화 계수
  \item $P_n$: $n$차 르장드르 다항식
  \item $\phi$: 지심위도(geocentric latitude), $\sin\phi = z/r$
\end{itemize}
...
\textbf{편미분 유도.} 계산의 편의를 위해 $r^2 = x^2 + y^2 + z^2$이므로 $\partial r/\partial x = x/r$ 등의 관계를 이용한다.
...
이 공식이 \texttt{j2\_perturbation.py}에 직접 구현되어 있다.


\subsection{Secular Perturbation: RAAN Drift Rate}

J2 섭동은 궤도 요소에 장기(secular) 변화를 유발한다. 특히 승교점경도(RAAN, $\Omega$)의 평균 변화율은 궤도역학에서 중요한 역할을 한다. 가우스 행성 운동 방정식(Gauss's variational equations)을 J2 섭동에 대해 한 주기 평균하면 다음을 얻는다 \cite{Vallado2013}:
\[
...
여기서:
\begin{itemize}
  \item $n = \sqrt{\mu/a^3}$: 평균 운동(mean motion)
  \item $p = a(1 - e^2)$: 반직현(semi-latus rectum)
  \item $i$: 궤도 경사각(inclination)
  \item $a$: 장반경(semi-major axis), $e$: 이심률
\end{itemize}
...
\textbf{유도 과정.} 가우스 행성 방정식에서 $\Omega$의 변화율은 다음과 같다:
\[
...
\textbf{수치 예시.} $h = 400\;\text{km}$, $i = 45°$ 원궤도에서:
\begin{itemize}
  \item $a = 6778.137\;\text{km}$
  \item $n = \sqrt{398600.4418 / 6778.137^3} = 1.1311 \times 10^{-3}\;\text{rad/s}$
  \item $\dot{\Omega} = -1.5 \times 1.1311 \times 10^{-3} \times 1.08263 \times 10^{-3} \times (6378.137/6778.137)^2 \times \cos(45°)$
  \item $\dot{\Omega} \approx -1.021 \times 10^{-6}\;\text{rad/s} \approx -5.06°/\text{day}$
\end{itemize}

이 해석식은 수치 검증(Sec.~\ref{sec:verification})에서 수치 적분 결과와의 비교 기준으로 사용된다.


\subsection{지수 대기 모델 (Exponential Atmosphere)}
...
여기서:
\begin{itemize}
  \item $h = \|\mathbf{r}\| - R_E$: 지표면 위 고도 [km]
  \item $\rho_0$: 기준 고도에서의 대기 밀도 [kg/m$^3$]
  \item $h_{\text{ref}}$: 기준 고도 [km]
  \item $H$: 대기 스케일 높이(scale height) [km]
\end{itemize}

실제 대기는 고도에 따라 온도와 조성이 변하므로 하나의 $(\rho_0, H)$ 쌍으로 전 고도를 표현할 수 없다. 본 프로젝트에서는 Vallado \cite{Vallado2013}의 층상 모델(layered model)을 채택하여, 고도를 여러 구간으로 나누고 각 구간마다 다른 계수를 사용한다.

\begin{table}[h]
\centering
\caption{지수 대기 모델 계수 (일부, Vallado \cite{Vallado2013} Table 8-4)}
\label{tab:atmo_params}
\begin{tabular}{cccc}
\toprule
...
여기서:
\begin{itemize}
  \item $C_D$: 항력 계수(drag coefficient), 무차원. 본 프로젝트에서는 $C_D = 2.2$ 사용
  \item $A/m$: 단면적 대 질량 비 [m$^2$/kg]. 본 프로젝트에서는 $A/m = 0.01\;\text{m}^2/\text{kg}$ 사용
  \item $\rho$: 대기 밀도 [kg/m$^3$] (Sec.~2.5에서 계산)
  \item $\mathbf{v}_{\text{rel}}$: 대기에 대한 상대 속도
\end{itemize}

\textbf{대기 공전 보정.} 대기는 지구와 함께 자전하므로, 우주선의 관성 속도 $\mathbf{v}$에서 대기의 공전 속도를 빼야 한다.
\[
...
\textbf{단위 변환.} 코드에서는 위치를 km, 속도를 km/s 단위로 사용한다. 항력 공식의 각 인자 단위를 추적하면:
\[
...
\begin{table}[h]
\centering
\caption{운동방정식의 가속도 성분 구성}
\label{tab:accel_components}
\begin{tabular}{ccc}
\toprule
가속도 성분 & 수식 & 조건 \\
\midrule
$\mathbf{a}_{\text{grav}}$ & $-\mu/r^3\,\mathbf{r}$ & 항상 포함 \\
$\mathbf{a}_{J2}$ & Sec.~2.3의 공식 & \texttt{include\_j2=True}일 때 \\
$\mathbf{a}_{\text{drag}}$ & Sec.~2.6의 공식 & \texttt{include\_drag=True}일 때 (NumPy 전용) \\
$\mathbf{u}$ & 추력 가속도 & 최적화 제어 입력 \\
\bottomrule
\end{tabular}
\end{table}

CasADi 심볼릭 버전에서는 대기 항력을 포함하지 않는다. 이는 \texttt{\_get\_atmo\_layer} 함수의 고도별 조건 분기(if-else)가 CasADi의 심볼릭 미분(automatic differentiation)과 호환되지 않기 때문이다. 최적화 문제에서 항력은 일반적으로 후처리(post-processing) 단계에서 고려하거나, 필요시 CasADi의 \texttt{if\_else} 함수로 별도 구현해야 한다.


\subsection{LEO 고도별 섭동 크기 비교}
...
\textbf{제3체 섭동 (달/태양):} LEO에서 달과 태양의 조석 가속도는 대략 다음과 같다 \cite{Montenbruck2000}:
\[
\|\mathbf{a}_{\text{Moon}}\| \sim 5 \times 10^{-6}\;\text{m/s}^2, \quad \|\mathbf{a}_{\text{Sun}}\| \sim 2 \times 10^{-6}\;\text{m/s}^2
\]

\textbf{섭동 크기 비교 요약 ($h = 400\;\text{km}$):}

\begin{table}[h]
\centering
\caption{LEO 400\,km 고도에서의 섭동 크기 비교}
...
J2 섭동은 이체 중력 대비 $10^{-3}$ 수준으로, 수 주기에 걸친 궤도전이에서 RAAN drift 등 유의미한 궤도 변화를 유발하므로 반드시 포함해야 한다. 대기 항력은 고도 400\,km 이상에서는 다른 섭동원보다 작지만, 저고도($< 300$\,km)에서는 밀도가 급증하여 중요해진다. 제3체 섭동과 태양 복사압은 단기 궤도전이(수 시간$\sim$수 일)에서는 영향이 미미하여 본 프로젝트에서 무시한다.

고도에 따른 항력 가속도 변화를 살펴보면:

\begin{table}[h]
\centering
\caption{고도에 따른 항력 가속도 변화}
\label{tab:drag_vs_altitude}
\begin{tabular}{cccc}
\toprule
...
\subsection{코드-수식 대응표}

\begin{table}[h]
\centering
\caption{수학 표현과 Python 구현의 대응}
\label{tab:code_mapping}
\begin{tabular}{llll}
\toprule
수학 표현 & Python 함수 & 파일 & 비고 \\
\midrule
$\mathbf{a}_{\text{grav}} = -\mu/r^3\,\mathbf{r}$ & \texttt{gravity\_acceleration(r, mu)} & \texttt{two\_body.py:8-28} & NumPy/CasADi 겸용 \\
$\mathbf{a}_{J2}$ (Sec.~2.3 공식) & \texttt{j2\_acceleration(r, mu, J2, R\_E)} & \texttt{j2\_perturbation.py:8-50} & NumPy/CasADi 겸용 \\
$\rho_0, h_{\text{ref}}, H$ 조회 & \texttt{\_get\_atmo\_layer(h)} & \texttt{drag.py:14-36} & 고도별 구간 검색 \\
$\mathbf{a}_{\text{drag}}$ (Sec.~2.6 공식) & \texttt{exponential\_drag(...)} & \texttt{drag.py:39-78} & NumPy 전용 \\
$\dot{\mathbf{x}} = f(\mathbf{x}, \mathbf{u})$ & \texttt{spacecraft\_eom\_numpy(...)} & \texttt{eom.py:12-38} & NumPy 전용, 전체 EOM \\
CasADi 심볼릭 EOM & \texttt{create\_dynamics\_function(...)} & \texttt{eom.py:41-68} & drag 제외, \texttt{ca.Function} 반환 \\
\bottomrule
\end{tabular}
\end{table}


\subsection{NumPy/CasADi 겸용 설계}

\texttt{gravity\_acceleration}과 \texttt{j2\_acceleration}은 동일한 수학 공식을 NumPy 배열과 CasADi 심볼릭 변수 양쪽에서 실행할 수 있도록 설계되었다. 입력 \texttt{r}의 타입을 런타임에 검사하여 분기한다.
...
이 설계를 통해:
\begin{itemize}
  \item \textbf{수치 적분}(RK4 등)에서는 NumPy 경로로 빠르게 실행
  \item \textbf{최적화}(direct collocation)에서는 CasADi 경로로 심볼릭 미분(AD) 지원
\end{itemize}

\subsection{대기 모델 구간 검색}

\texttt{\_get\_atmo\_layer} 함수는 고도 $h$를 입력받아 \texttt{ATMO\_PARAMS} 테이블에서 해당 구간의 $(\rho_0, h_{\text{ref}}, H)$를 반환한다. 선형 탐색으로 구현되어 있으며, 모델 범위(150--1100\,km) 밖의 고도에 대해서는 \texttt{ValueError}를 발생시킨다.
...
모든 물리 상수는 \texttt{constants.py}에서 중앙 관리된다.
...
상수 & 변수명 & 값 & 단위 \\
\midrule
지구 중력 상수 & \texttt{MU\_EARTH} & 398600.4418 & km$^3$/s$^2$ \\
지구 적도 반지름 & \texttt{R\_E} & 6378.137 & km \\
J2 계수 & \texttt{J2} & $1.08263 \times 10^{-3}$ & 무차원 \\
항력 계수 & \texttt{CD} & 2.2 & 무차원 \\
단면적/질량 비 & \texttt{AREA\_MASS\_RATIO} & 0.01 & m$^2$/kg \\
지구 자전 각속도 & \texttt{OMEGA\_EARTH} & $7.2921159 \times 10^{-5}$ & rad/s \\
\bottomrule
\end{tabular}
\end{table}

이 값들은 WGS-84 및 Vallado \cite{Vallado2013}를 기준으로 한다.
...
수치 검증은 \texttt{tests/test\_dynamics.py}에 구현되어 있으며, 4가지 테스트로 구성된다.

\subsection{원궤도 1주기 전파}

\textbf{목적:} 수치 적분기와 동역학 모델의 기본 정확도를 확인한다.

\textbf{방법:} 적도 원궤도($h = 400\;\text{km}$)에서 $\mathbf{u} = \mathbf{0}$으로 1 케플러 주기를 RK4로 전파한다.

\begin{itemize}
  \item \textbf{J2 미포함 케이스:} 이체 문제의 정확해로 원궤도가 폐곡선이므로, 1주기 후 초기 상태로 복귀해야 한다.
  \begin{itemize}
    \item 판정 기준: 위치 오차 $< 10^{-8}\;\text{km}$, 속도 오차 $< 10^{-10}\;\text{km/s}$
    \item RK4 스텝 수: 10,000
  \end{itemize}
  \item \textbf{J2 포함 케이스:} J2 secular drift로 인해 카르테시안 상태의 완전 복귀는 기대할 수 없다. 대신 궤도 반경($\|\mathbf{r}\|$)이 보존되는지 확인한다.
  \begin{itemize}
    \item 판정 기준: 궤도 반경 변화 $< 0.1\;\text{km}$, 속력 변화 $< 10^{-4}\;\text{km/s}$
  \end{itemize}
\end{itemize}

\subsection{에너지 보존}

\textbf{목적:} 이체 문제($J2$ 미포함, $\mathbf{u} = \mathbf{0}$)에서 수치 적분의 에너지 보존 성능을 확인한다.

\textbf{방법:} 적도 원궤도에서 10주기(약 15.3시간)를 전파하며 매 주기 끝에서 비에너지를 계산한다.
\[
\mathcal{E} = \frac{1}{2}\|\mathbf{v}\|^2 - \frac{\mu}{\|\mathbf{r}\|}
\]

\begin{itemize}
  \item 판정 기준: $|\Delta\mathcal{E}| < 10^{-10}\;\text{km}^2/\text{s}^2$
  \item RK4 총 스텝 수: 50,000 (주기당 5,000)
\end{itemize}
...
\textbf{목적:} 동일한 입력에 대해 NumPy 경로와 CasADi 경로가 동일한 결과를 산출하는지 확인한다.

\textbf{방법:} 세 가지 입력 조합에서 \texttt{spacecraft\_eom\_numpy}와 \texttt{create\_dynamics\_function}의 출력을 비교한다.
\begin{enumerate}
  \item 적도 원궤도, J2 미포함, 추력 없음
  \item 경사 원궤도($i = 45°$), J2 포함, 추력 없음
  \item 경사 원궤도($i = 30°$, $h = 500\;\text{km}$), J2 포함, 추력 포함
\end{enumerate}

\begin{itemize}
  \item 판정 기준: 모든 성분에서 최대 오차 $< 10^{-14}$
  \item 이 수준은 부동소수점 산술의 기계 엡실론(machine epsilon, $\sim 2.2 \times 10^{-16}$)에 가까운 정밀도이다.
\end{itemize}

\subsection{J2 RAAN Drift}

\textbf{목적:} J2 구현의 물리적 정확도를 검증한다. 수치 적분으로 관측된 RAAN 변화량이 해석식(Sec.~2.4)의 예측과 일치하는지 확인한다.

\textbf{방법:}
\begin{itemize}
  \item 초기 조건: $h = 400\;\text{km}$, $i = 45°$ 원궤도 ($\Omega_0 = 0$, ascending node가 $x$-축)
  \item 전파 시간: 10 케플러 주기
  \item RK4 스텝 수: 100,000
\end{itemize}
