...
The demonstration uses the IRLS-weighted L1-style control-effort surrogate objective defined in Section 4.2. No alternative objective mode is reported as a paper-level result. An additional prograde-preservation constraint is enabled across all reported runs, enforcing that the linearized angular-momentum projection remains non-negative at interior parameter samples. Because this constraint is configuration-specific rather than central to the method's general claim, it is disclosed here for reproducibility: all reported trajectories in Section 6 are generated with this constraint active.

The solver backend uses a Rust-based QP implementation. The SCP outer loop is initialized from a straight-line control polygon between the endpoints and runs for a maximum of 500 outer iterations with a convergence tolerance of $\|P^{(k+1)} - P^{(k)}\|_F < 10^{-6}$. A proximal regularization weight of $10^{-6}$ and a control-point step-clipping radius of 2000 km are used across all reported runs.
