...
- Good pattern example:
  - `제어점의 컨벡스 헐 안에 놓인다.`
- **업데이트 (2026-08): 이 정책은 폐기되었다.** 교수 지시로 convex 계열은 `볼록`으로 통일하며(볼록 QP, 비볼록, 볼록화, 볼록 껍질), 저자가 2026-08-05 재확정하였다. 커뮤니티 관행은 컨벡스(2025 춘계 KSAS 논문집 컨벡스 17회 : 볼록 0회)임을 §6에 기록해 둔다.
...
### Case 33. 전문 용어의 사전식 조어 (term-of-art coinage)

- Source pattern:
  - `수락`, `수락 판정`, `수락 규칙`, `수락 임계값`, `벌점`, `벌점 함수`, `벌점 가중치`, `갱신`, `수락된 갱신`, `위반량`, `부분 문제`, `신뢰 반경`, `실행 가능성 복원 단계`, `목적 감소량`
- Comment (저자):
  - `I would not use these words as the meaning they are being used for.`
- Bad pattern:
  - SCvx/신뢰영역 계열의 term of art를 영한사전 대응으로 직역해 조어함. 만들어진 단어들은 한국어에서 이미 다른 어휘장에 산다 — 수락(제안 수락), 벌점(운전면허 벌점), 갱신(면허 갱신), 위반(교통 위반), 판정(심판 판정) — 모아 놓으면 수학이 아니라 행정·법규 문서처럼 읽힘
  - Koranglish의 정확히 반대 방향 과잉 교정: "한국어 우선"을 전역 규칙으로 오적용한 결과
- Better direction:
  - 낯익음("해당 독자층에 익숙한가") 판정은 문서 단위가 아니라 **용어 단위**로 한다
  - 인쇄된 국내 선례를 먼저 찾는다 (KoreaScience, KCI, 학회 논문집). 선례가 없으면 조어하지 않고 영어를 유지한다
  - Case 29의 규칙("한자어 또는 영어, 조어 금지")이 정확히 이 상황의 규칙이다
- Good pattern example:
  - §6 용어 판정표 참조

### Case 34. 국내 선례가 없어 보이는 서술은 인접 분야의 인쇄 선례를 차용

- Source pattern:
  - ρ-test(후보 해 accept/reject)를 서술할 관행이 국내 GNC SCP 논문에 없음 → `수락 규칙` 등을 발명
- Comment:
  - 국내 SCP 논문들은 고정/soft 신뢰 구간을 쓰므로 수락 판정 서술 자체가 드묾. 그러나 인접 분야(순차적 근사 최적설계)에 정확한 선례가 인쇄되어 있었음
- Bad pattern:
  - "선례가 없다"고 판단되는 즉시 조어로 감. 인접 분야·고전층(최적설계, OR 교과서) 논문을 검색하지 않음
- Better direction:
  - 오민환 외, 한국항공우주학회지 39(4), 2011: "신뢰영역 비는 … 예측 감소량에 대한 실제 감소량의 비로 계산된다", "신뢰영역 비 ρ가 0보다 크면 새로운 설계점으로 간주하며", "신뢰영역의 크기 ∆를 조절하게 된다"
  - accept 동사 자체가 필요 없다: "기준을 넘으면 새로운 기준점으로 간주한다"
- Good pattern example:
  - `ρ가 기준을 넘으면 하위 문제의 해를 새로운 기준점으로 간주하고, 그렇지 않으면 신뢰 구간의 크기를 줄여 같은 기준점에서 다시 푼다.`
  - (`후보 해`도 조어에 가깝다 — 국내 논문은 candidate 명사 없이 `하위 문제의 해`/`근사 최적해` + 기호로 서술한다)
...
15. ~~`convex` 계열 용어는 저자 정책에 따라 `컨벡스 / 컨벡스 헐 / 논컨벡스 / 컨벡스화`로 통일한다.~~ **폐기(2026-08):** 교수 지시로 `볼록` 계열로 통일한다(Case 26 업데이트, §6 참조). 혼용 금지 원칙만 유지.
...
19. 전문 용어(term of art)를 사전 대응으로 조어하지 않는다. 낯익음 판정은 용어 단위로 하고, 인쇄된 국내 선례(KoreaScience·KCI·학회 논문집)를 먼저 찾는다. 선례가 없으면 영어를 유지한다. (Case 33)
20. 해당 분야에 선례가 없어 보이면 인접 분야와 고전층(순차적 근사 최적설계, OR 교과서)까지 검색한 뒤에 판단한다. (Case 34)
...
- Bad: `후보 해를 수락한다`, `수락 임계값`
- Good: `비 ρ가 기준을 넘으면 새로운 기준점으로 간주한다`
...
## 6. SCP/SCvx 용어 판정표 (2026-08 corpus case study)

배경: §3.3 재작성 중 조어 문제(Case 33)가 드러나 국내 인쇄 코퍼스 3종을 조사하였다 — (1) 학위논문·교과서·대한수학회/한국통계학회 용어집, (2) 항공우주 GNC 논문 12편(한국항공우주학회지 원문, KSAS 학술대회 논문집 2024–2026 전권), (3) 제어·전력·산업공학 학회지 11편. 원칙: 인쇄된 문장만 근거로 삼는다.

**저자 확정 (2026-08-05): 신뢰 구간 · 볼록 · 실현 가능.**

| 개념 | 채택 | 인쇄 근거 |
|---|---|---|
| trust region | **신뢰 구간** (저자 확정) | "신뢰 구간(Trust-Region)이 잘 정의되고" — 정기욱, 2022 항공우주논문상 우수상; "신뢰구간을 구속조건으로 추가해" — 정철구·이창훈·탁민제, 군사과학기술학회 2021; "신뢰구간과 수렴 조건은 다음과 같이 설정하였다" — 권혁훈 외, 전기학회논문지 68(1) 2019. 소수형: 신뢰영역(한국항공우주학회지 39(4) 2011). `신뢰 반경`은 0건 — 크기는 "신뢰 구간의 크기" |
| ratio test (accept/reject) | 비 ρ가 기준을 넘으면 **"새로운 기준점으로 간주"** | "신뢰영역 비 ρ가 0보다 크면 새로운 설계점으로 간주하며" — 오민환 외, 한국항공우주학회지 39(4) 2011. `수락`은 GNC 인쇄물 0건 |
| predicted / actual reduction | **예측 감소량 / 실제 감소량** | "예측 감소량에 대한 실제 감소량의 비로 계산된다" — 같은 논문 |
| subproblem | **하위 문제** | "하위 문제 𝑃를 컨벡스한 형태로 변형할 수 있다" — 정기욱; "하위 문제를 생성함으로써" — 고효상 외, 한국항공우주학회지 50(11) 2022. 고전층: 부문제(이태희, 한국CDE학회지 2004). `부분 문제` 0건 |
| slack variable / virtual control | **여유 변수** (첫 등장 slack 병기). virtual control은 조어하지 않고 **"실현 가능성을 보완해주는 항"**으로 서술 | "ν: 여유 변수" — 고효상 외; "여유변수(slack …)" — 권혁훈 외 2019; "ν1, ν2 는 이산화된 동역학식의 실현가능성을 보완해주는 항이고" — 정철구 외. `가상 제어` 0건 |
| penalty | **페널티 항 / 페널티 계수** | "ℓ1 페널티 항의 총합이 0으로 수렴하여" — 고효상 외; "페널티 계수 (C)는 10⁴으로 설정하였다" — 전기학회논문지 59(5) 2010. `벌칙함수`는 고전 최적설계·FEM, `벌점`은 통계·CS 계열 |
| iteration | **반복(iteration)** 첫 등장 병기, 이후 `매 반복`·`반복 횟수`; k번째 한 바퀴는 **시행**도 인쇄됨 | "윗첨자 i 는 반복(iteration) 횟수를 의미한다" — 정철구 외; "6번의 시행으로", "매 시행 오차를 기반으로" — 고효상 외 |
| constraint violation | **제약 조건 위반** (양: 위반양) | "제약조건 위반을 억제하는" — 제어로봇시스템학회 논문지 31(2) 2025; "제약조건 위반양에 해당하는 값" — 전기학회논문지 2010. `위반량` 0건 |
| feasible | **실현 가능(성)** (저자 확정; 기존 `실행 가능`은 교체 대상) | "실현 가능 해 집합은 컨벡스 집합이 되며" — 정기욱; "실현 불가능성을 방지하고" — 고효상 외. `실행 가능`은 GNC 코퍼스 0건 |
| threshold | **임계값** | "특정 임계값보다 낮도록" — 제어로봇시스템학회 논문지 2025. `문턱값` 0건 |
| convergence criterion | **수렴 조건** (산문); 의사코드 라벨로는 `수렴 판정`도 인쇄됨 | "수렴 조건은 다음과 같이 설정하였다" — 권혁훈 외 2021; "Step4: 수렴 판정" — 전기학회논문지 2010 |
| convex | **볼록** (교수 지시, 저자 재확정) | 커뮤니티 관행은 컨벡스가 압도적(2025 춘계 KSAS 논문집 컨벡스 17 : 볼록 0; "순차 컨벡스 프로그래밍" — 한국항공우주학회지 50(11) 2022 표제). 본 논문은 교수 지시에 따라 볼록으로 통일하고, 이 divergence를 여기 기록해 둔다 |
| merit function | **영어 유지만** ("확대 목적함수" 금지 — 저자 기각 2026-08-06) | 전기학회논문지 2010의 "확대 목적함수" 용례는 타분야 단일 용례로 낯익음 요건 미달. 저자가 조어로 읽고 기각 — 단일 인쇄 용례는 채택 근거가 못 되며, 판정표에 넣기 전에 대상 독자층 다수 용례를 요구할 것 |
| update | 신뢰 구간: `수정`/`업데이트`; 반복점: "새로운 기준점으로 간주" 우선. `갱신`은 유도해·주기 갱신 맥락 | "매 시행 오차를 기반으로 신뢰 구간을 수정하여" — 고효상 외; "TR 업데이트 방식" — 인하대, KSAS 2025 춘계; "착륙 유도의 해가 갱신될 때" — 정기욱 |
| SOCP | 2차 원뿔 프로그래밍 / 이차 원뿔 계획법 | 권혁훈 외 2021; 고효상 외 2022; KSAS 2024 추계 |

원문 출처 일람은 세 조사 보고서의 인용 링크를 따르며, 대표 원문: 오민환 외(한국항공우주학회지 39(4), 2011, KoreaScience JAKO201115037883446), 정기욱(2022 항공우주논문상, KAI 공개 PDF), 고효상·최한림 외(한국항공우주학회지 50(11), 2022, JAKO202234060798024), 권혁훈 외(전기학회논문지 68(1), 2019, JAKO201911562300791 / 한국항공우주학회지 49(1), 2021), 이태희(한국CDE학회지 10(1), 2004, JAKO200411922464249).
