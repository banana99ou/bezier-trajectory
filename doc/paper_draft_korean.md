...
> **(증명 요약)[[awkward direct translation]].** Bézier 곡선은 제어점의 (볼록 껍질)[[wrong word choice. use 컨벡스 헐]] 안에 놓인다. 구의 지지 반공간은 구의 내부를 배제하면서 경계에 접한다. 따라서 모든 제어점이 $\mathcal{H}^{(s)}$ 안에 있으면 볼록 껍질 전체도 $\mathcal{H}^{(s)}$ 안에 있고, 곡선도 $\mathcal{H}^{(s)} \cap \mathcal{K}^c$ 안에 놓인다. $\square$
...
반복 한도 도달은 수렴 허용오차($\|P^{(k+1)} - P^{(k)}\|_F < 10^{-12}$)가 구조적으로 도달하기 어려운 수준이라는 점에서 (유래한다)[[unusual word choice]]. (SCP 절차)[[awkward]]에서는 매 반복마다 지지 반공간과 중력 선형화가 현재 해를 기준으로 재구성되므로, 선형화 기준점이 반복마다 이동하여 제어점 변화의 Frobenius (노름)[[da fuq is this brain dead word? I know Korean people use this word. this feels soooo brain dead Konglish. either use 한자어 or English]]이 $10^{-12}$ 이하로 감소하기 어렵다. 실질적 수렴 여부를 확인하기 위해, 대표 설정($N=7$, $n_{\mathrm{seg}}=16$)에서 반복 한도를 100부터 10000까지 (변화시키는 진단 실험)[[bad word choice. choose other words for this meaning]]을 수행하였다. 반복 한도 1000에서 delta-v 대리 지표는 10000회 기준값 대비 약 2.6%, (안전 여유는 약 7% 이내에 있었고)[[again reuse of English sentence structure]], 5000회에서는 각각 0.23%와 1.5% 이내로 수렴하였다. 따라서 10000회 반복 한도 도달은 (해의 발산이 아니라 실질적 수렴 이후의 미세한 점진적 개선에 해당하며)[[again reuse of English sentence structure. not A but B.]], (보고된)[[another brain dead direct translation of "reported"]] 결과는 안정화된 해로 해석할 수 있다.
...
여기서 보수성(conservatism)이란, 지지 반공간 구성이 부과하는 안전 여유와 곡선의 실제 최소 접근 거리 사이의 차이를 가리킨다. 이 차이는 제어다각형의 (볼록 껍질)[[sync word choice with the rest of paper. my choice of word for convex is "컨벡스"]]이 곡선 자체보다 넓은 영역을 차지하기 때문에 발생한다. 분할 수가 증가하면 각 분할구간이 짧아지고 제어다각형이 곡선에 더 가까워지므로, 지지 반공간 제약이 실제 곡선-KOZ 거리를 보다 정밀하게 반영하게 된다. 표 3의 안전 여유 열은 이 보수성의 직접적인 척도이며, $n_{\mathrm{seg}}=4$의 약 144 km에서 $n_{\mathrm{seg}}=64$의 약 0.9 km로의 단조 감소가 이를 확인해 준다.
