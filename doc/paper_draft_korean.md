...
본 논문에서는 구형 Keep-Out Zone(KOZ)을 연속적으로 회피하는 Bézier 기반 궤적 초기화 기법을 제안한다. 제안 방법은 전적으로 제어점(control point) 공간에서 작동하며, 미분 연산자, 분할(subdivision) 행렬, 경계조건, KOZ 제약을 모두 제어점에 대한 선형 연산으로 표현한다. 구형 KOZ 회피는 De Casteljau 분할로 곡선을 여러 분할구간(sub-arc)으로 나눈 뒤, 각 분할구간의 제어다각형에 지지 반공간(supporting half-space) 제약을 부과하는 방식으로 보수적으로 처리한다. Bézier 곡선의 볼록 껍질(convex hull) 성질에 따르면 해당 분할구간의 모든 제어점이 반공간 제약을 만족할 때 곡선 전체도 KOZ 바깥에 놓이게 된다. 최종 최적화는 순차 컨벡스화(successive convexification) 절차 안에서 일련의 convex QP를 푸는 방식으로 수행된다.

제안한 기법을 검증하기 위하여 단순화된 orbital transfer 문제에 적용하였다. 분할 수 변화 (실험 결과, 분할 수가 증가할수록 계산 시간은 증가하지만 본 실험 설정에서는 안전 여유와 delta-v 대리 지표의 변화는 크지 않았다.)[[this section should be a bolier plate. remember. results are not finalized yet.]] Bézier 차수 변화 실험에서는 모든 시험 차수에서 실행 가능한 궤적을 얻을 수 있었으며, 고차에서는 목적함수 값이 소폭 개선될 수 있으나 계산 비용이 뚜렷하게 증가하였다. 제안한 방법은 후속 solver에 전달할 수 있는 매끄럽고 연속적으로 안전한 warm start 초기값을 생성하는 데 초점을 둔다.
...
본 절에서는 제안 기법을 세 가지 맥락에서 (위치시킨다.)[[unnatural]] 첫째는 direct transcription 및 direct collocation 계열의 궤적 최적화 방법, 둘째는 obstacle avoidance를 위한 컨벡스화 또는 보수적 안전 처리 방식, 셋째는 후속 최적화를 위한 초기화와 warm start 생성 방법이다.

### 2.1 Direct transcription 및 direct collocation과의 관계

Direct transcription과 direct collocation은 제약이 있는 궤적 최적화에서 가장 널리 쓰이는 (접근이다.)[[bad direct translation of approach]] 이들 방법은 궤적을 여러 노드에서의 상태와 입력 변수로 이산화하고, dynamics를 equality constraint로 부과한 뒤, 큰 규모의 nonlinear program을 푼다. 다양한 문제에 적용 가능하고 solver 생태계도 잘 갖추어져 있다는 장점이 있다.

본 연구는 이러한 방법을 대체하려는 것이 아니라, 그 전에 사용할 수 있는 구조화된 초기 궤적을 만드는 문제에 초점을 둔다. 즉, 본 논문의 관심은 direct collocation 자체의 대체 가능성보다는 제어점 공간 정식화가 후속 solver에 유용한 초기값을 제공할 수 있는가에 있다.

### 2.2 보수적 장애물 처리 및 컨벡스화

(연속적인 장애물 회피는 점별 제약만으로는 다루기 어렵다. 노드 간 구간에서 어떤 일이 일어나는지는 추가 논증이 필요하기 때문이다.)[[weak or possibly wrong explanation]]이를 다루기 위한 여러 접근 가운데 일부는 특정 문제 클래스에 대해 무손실 컨벡스화를 사용하고, 또 다른 일부는 순차 컨벡스화로 논컨벡스 제약을 반복적으로 선형화한다.
...
본 논문의 제안 기법은 이러한 초기화 방법들 가운데 하나로 이해할 수 있다. 즉, 비교적 저차원인 제어점 공간에서 매끄럽고 안전한 궤적을 먼저 만든 다음, 이를 후속 solver의 warm start 초기값으로 제공하는 방식이다. 여기서 장점은 초기화 문제의 차원을 작게 유지하면서도 기하학적 안전 제약을 명시적으로 다룰 수 있다는 점이다.
...
단순화된 orbital transfer 문제에 대한 실험 결과, 제안 기법은 시험한 차수와 분할 설정 전반에서 실행 가능한 궤적을 생성할 수 있었다. (분할 수를 늘리면 계산 시간은 증가했지만, 현재 실험에서는 안전 여유와 delta-v 대리 지표의 변화는 크지 않았다. 차수 증가 역시 목적함수 측면에서 소폭 이득을 줄 수 있었으나, 계산 비용 증가가 함께 나타났다. 따라서 본 연구의 결과는 분할 수와 차수 모두에 대해 단순한 우열보다는 trade-off 관점에서 이해하는 것이 적절하다.)[[should be boillerplate not actual intepretation of result.]]

결론적으로, 제안 기법은 제어점 공간에서 연속 안전 제약을 구성하고 이를 SCP 기반 최적화와 결합하는 하나의 정식화를 제공한다. 향후에는 후속 solver와의 연계 검증, 다양한 문제 설정에 대한 실험 확대, 그리고 시간 최적화를 포함하는 확장을 통해 적용 범위를 넓힐 수 있다.
...
- [ ] Positioning cluster: 초록/서론/관련연구/결론 전반에서 framework-first positioning을 일관되게 재정렬할 것. 구체적으로는 non-claim 문장의 전면 배치 여부, `warm start`와 후속 solver의 위상, direct collocation 관련 단락의 비중, 그리고 결론에서 무엇을 먼저 말할지를 하나의 묶음으로 다시 설계할 것.
