...
제안된 기법을 단순화된 orbital transfer 문제에 적용하여 검증하였다. 분할 수와 Bézier 차수에 대한 변화 실험을 통해 계산 비용, 안전 여유, 그리고 대리 목적함수의 변화를 비교함으로써 정식화의 거동을 정량적으로 살펴보았다. 제안한 프레임워크는 후속 solver에 전달할 수 있는 매끄럽고 연속적으로 안전한 warm start 초기값 생성에도 intended use를 가지지만, 이에 대한 matched downstream comparison은 현재 draft에서 placeholder로 남겨 둔다.
...
로 둔다. 따라서 실제 속도와 가속도는 $\tau$에 대한 미분에 각각 $1/T$, $1/T^2$를 곱한 형태로 얻어진다. 본 정식화에서는 전이 시간 $T$를 고정하며, 이에 따라 free final time이나 timing allocation은 현재 범위에 포함하지 않는다.
...
**[T1] Control-point-space objects and linear maps (draft placeholder).**

[추후 삽입: decision variable, derivative operators, subdivision operators, boundary rows, KOZ constraint rows를 한 표로 정리하여 각 연산의 차원, 정의, 역할을 요약한다.]
...
**그림 1 [F1].** De Casteljau 분할로 얻은 분할구간과 지지 반공간을 이용한 연속 안전 제약의 개념도.
...
**[F2] SCP pipeline in control-point space (draft placeholder).**

[추후 삽입: initialize control points -> assemble derivative/objective operators -> subdivide into sub-arcs -> build supporting half-spaces -> solve convex QP -> update iterate 의 흐름도를 제시한다.]
...
시연 시나리오는 Progress-to-ISS 접근 문제를 단순화한 단일-arc orbital transfer 설정이다. Chaser는 고도 245 km의 원궤도에서 시작하고, target은 고도 400 km의 원궤도에 있다고 가정한다. 두 궤도는 동일한 평면과 51.64 deg 경사각을 공유하며, 초기 위상차는 70 deg로 둔다.

Solver는 Rust 기반 QP 백엔드를 사용하였다. SCP는 직선 형태의 초기 제어다각형에서 시작하며, 최대 10000회의 반복까지 수행한다. 수렴 허용오차는 $\|P^{(k+1)} - P^{(k)}\|_F < 10^{-6}$으로 두었고, proximal regularization weight는 $10^{-6}$, step clipping 반경은 2000 km로 고정하였다.
...
첫 번째 변화 실험은 Bézier 차수를 $N=7$으로 고정한 상태에서 분할 수 $n_{\mathrm{seg}} \in \{2,4,8,16,32,64\}$를 변화시키는 실험이다. 목적은 분할 수가 커질수록 계산 비용과 안전 근사 정도 사이에 어떤 변화가 나타나는지 확인하는 것이다.

두 번째 변화 실험은 차수 $N \in \{6,7,8\}$에 대한 비교이다. 대표 비교 표는 $n_{\mathrm{seg}} = 16$에서 구성하였고, 전체 분할 수 변화 실험에 대해서도 차수별 추세를 함께 확인하였다. 차수 비교에는 표현 자유도 변화와 변수 수 변화가 동시에 반영되므로, 결과는 계산 비용과 표현력의 결합된 효과로 해석한다.
...
![그림 2. 대표 orbital transfer 궤적 예시](../figures/f3_representative_settings.png)
**그림 2 [F3].** 대표 orbital transfer 설정에서 얻은 representative trajectory 예시.
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| `demo_N6_seg16` | 6 | 7 | 16 | True | 145.000 | 6,269.279 | 64.705 | 10000 |
| `demo_N7_seg16` | 7 | 8 | 16 | True | 145.000 | 5,883.266 | 78.276 | 10000 |
| `demo_N8_seg16` | 8 | 9 | 16 | True | 145.000 | 5,812.755 | 94.545 | 10000 |

표 2에서 보듯이, 시험한 세 차수 모두에서 실행 가능한 궤적을 얻을 수 있었다. 안전 여유는 세 경우 모두 약 145 km로 동일하였다. Delta-v 대리 지표는 $N=6$에서 약 6,269 m/s, $N=7$에서 약 5,883 m/s, $N=8$에서 약 5,813 m/s로 감소하므로, 대표 설정에서는 차수가 증가할수록 목적함수 값이 다소 개선되는 경향이 나타난다. 반면 계산 시간은 $N=6$에서 약 64.7 s, $N=7$에서 약 78.3 s, $N=8$에서 약 94.5 s로 증가하였다. 따라서 대표 설정에서는 $N=8$이 가장 낮은 delta-v 대리 지표를 주지만, 그 대가로 계산 시간도 가장 크다. 또한 세 경우 모두 10000회의 SCP 반복 한계에 도달하여 종료되었으므로, 이 비교는 완전 수렴 순위라기보다 terminal iterate 기준의 tradeoff로 해석해야 한다.

### 6.2 분할 수에 따른 변화

다음으로 분할 수가 결과에 미치는 영향을 살펴본다. 정량 결과는 표 3에 정리하였다.

**표 3 [T3]. 분할 수에 대한 변화 실험 결과 ($N=7$)**

| Setting | Degree | Control points | $n_{\mathrm{seg}}$ | Solve success | Safety margin (km) | Delta-v proxy $J_{\mathrm{dv}}$ (m/s) | Runtime (s) | iterations |
|---:|---:|---:|---:|---:|---:|---:|
| 7 | 2 | False | -989.490 | 17,378.595 | 0.262 | 1 |
| 7 | 4 | True | 145.000 | 5,883.266 | 43.776 | 10000 |
| 7 | 8 | True | 145.000 | 5,883.266 | 56.151 | 10000 |
| 7 | 16 | True | 145.000 | 5,883.266 | 78.276 | 10000 |
| 7 | 32 | True | 145.000 | 5,883.266 | 113.609 | 10000 |
| 7 | 64 | True | 145.000 | 5,883.266 | 201.121 | 10000 |

표 3에서 가장 먼저 눈에 띄는 점은 가장 거친 설정인 $n_{\mathrm{seg}}=2$가 infeasible하다는 것이다. 이 경우 safety margin이 크게 음수로 나타나며, 해당 subdivision 수준은 이 시나리오에서 충분한 안전 근사를 제공하지 못한다. 반면 $n_{\mathrm{seg}} \ge 4$에서는 실행 가능성이 회복되고, 이후에는 safety margin이 약 145 km로 거의 일정하게 유지된다. Delta-v 대리 지표도 feasible 구간에서는 사실상 거의 변하지 않는다. 따라서 이 변화 실험은 분할 수 증가가 coarse feasibility threshold를 넘기는 데는 중요하지만, 그 이후 추가 subdivision이 강한 성능 개선을 주는 것은 아니라는 점을 보여준다. 대신 계산 시간은 $n_{\mathrm{seg}}$가 증가할수록 크게 증가하므로, 현재 데이터는 post-feasibility 구간에서의 강한 conservatism reduction보다는 runtime growth를 더 분명히 지지한다.

**[F4] Runtime and outcome trends versus subdivision count (draft placeholder).**

[추후 삽입: subdivision count에 따른 runtime, safety margin, Delta-v proxy trend를 패널 형태로 요약한 figure.]

### 6.3 Bézier 차수에 따른 변화

이제 Bézier 차수 변화가 결과에 미치는 영향을 살펴본다. 차수가 높아지면 표현 자유도는 커지지만, 변수 수와 계산량도 함께 증가한다. 정량 결과는 표 4에 정리하였다.

**표 4 [T4]. 차수에 대한 변화 실험 결과 ($n_{\mathrm{seg}}=16$)**
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| `demo_N6_seg16` | 6 | 7 | 16 | True | 145.000 | 6,269.279 | 64.705 | 4.228 |
| `demo_N7_seg16` | 7 | 8 | 16 | True | 145.000 | 5,883.266 | 78.276 | 4.000 |
| `demo_N8_seg16` | 8 | 9 | 16 | True | 145.000 | 5,812.755 | 94.545 | 3.960 |

표 4는 차수 변화에 따른 성능 지표를 비교한 것이다. 세 차수 모두 실행 가능성을 유지하며 safety margin도 동일한 수준을 보인다. 그러나 delta-v 대리 지표는 $N=6 \rightarrow 7 \rightarrow 8$로 갈수록 점차 감소하고, 평균 제어 가속도도 함께 낮아진다. 반면 계산 시간은 같은 순서로 증가한다. 즉, 높은 차수는 목적함수 측면에서 다소 더 좋은 값을 주지만, 그 대가로 runtime penalty도 함께 증가한다. 따라서 차수 변화의 효과는 blanket superiority가 아니라, 표현 자유도와 계산 비용 사이의 tradeoff로 해석하는 것이 적절하다.

**[F5] Multi-order performance trends (draft placeholder).**

[추후 삽입: degree별 runtime, Delta-v proxy, 그리고 선택된 smoothness metric의 trend figure.]
...
셋째, 실험은 단일 orbital transfer 예제에 기반한다. 정식화 자체는 기하학적으로 더 일반적인 구조를 가지더라도, 현재 단계의 실험적 근거는 해당 시연 문제에 대해 제시된다.
...
단순화된 orbital transfer 문제에 대한 실험 결과, 제안 기법은 대표 차수 설정 $N \in \{6,7,8\}$에서 실행 가능한 궤적을 생성할 수 있었고, subdivision 실험에서는 가장 거친 설정인 $n_{\mathrm{seg}}=2$가 infeasible인 반면 $n_{\mathrm{seg}} \ge 4$에서는 실행 가능성이 회복됨을 확인하였다. 또한 차수 증가에 따라 목적함수 값은 다소 개선되지만 계산 시간도 함께 증가하는 tradeoff가 나타났다.
현재 데이터는 제안 정식화의 feasibility와 차수-계산비용 tradeoff는 뒷받침하지만, downstream warm-start 효과는 아직 planned comparison 단계에 머물러 있다.
