#r세그먼트별 컨벡스화에 기반한 Bézier 궤적 초기화 기법
...
(5. 동일한 downstream multi-phase LGL Pass 2 solver·dynamics·tolerance 하에서 two-pass direct collocation pipeline의 Pass 1 단계를 Bézier SCP upstream으로 대체하는 matched pipeline-variant 비교 실험을 수행하여, 정의된 작동 영역 내에서 최종 비용이 보존되고 end-to-end runtime이 일부 사례에서 감소함을 확인한다.)[[feels too long. and too much detailed]]

(위 다섯 번째 기여는 본 논문에서 주장하지 않는 범위를 명확히 하기 위해 별도로 강조한다. 이 실험은 direct collocation 대비 method-class 우월성 주장이 아니라, 동일한 DCM pipeline 내부에서 Pass 1 초기화 단계를 대체했을 때의 거동을 측정한 것이며, 결과의 해석은 Bézier upstream이 feasible하고 downstream Pass 2가 수렴하는 circular orbit 영역으로 한정된다.)[[is this line really neccessary? even if it is neccessary, shouldn't it move to section 7?]]
...
- **Baseline (full two-pass DCM)**: Pass 1로 Hermite-Simpson collocation을 사용하여 thrust profile과 phase 구조를 구하고, peak detection 절차로 phase 경계를 결정한 뒤, Pass 2로 multi-phase Legendre-Gauss-Lobatto collocation을 수행한다.
- **Proposed (Bézier-replaces-Pass-1)**: Pass 1을 본 논문의 Bézier SCP optimizer (degree 6, $n_{\mathrm{seg}}=16$)로 대체한다. Peak detection, phase 구조 결정, Pass 2 transcription, dynamics 모형, IPOPT solver tolerance, boundary-condition protocol은 baseline과 동일하게 유지한다.
...
문제 (인스턴스)[[unnatural loneword used]]는 trajectory database `data/trajectories.duckdb`의 converged 행에서 추출하였다. 두 가지 run을 수행한다. 첫째는 $T_{\mathrm{normed}} \le 0.5$ 및 $\max(e_0, e_f) \le 0.1$ 조건의 10개 사례를 대상으로 하는 eccentricity-boundary run이고, 둘째는 default circular-ish 필터 ($\max \mathrm{ecc} \le 0.01$) 하의 converged 행 전체(112개)를 대상으로 하는 boundary sweep run이다. 보고 지표는 baseline과 proposed 각각의 수렴 여부, 단계별 wall-clock runtime, 최종 비용 $|\Delta \mathrm{cost}|$, peak 수, end-to-end speedup (baseline_time / proposed_total_time)이다. 실패 사례와 Bézier upstream infeasible 사례도 함께 기록한다.
...
(이상의 결과는 제안 정식화 자체의 실행 가능성과 분할 수 및 차수 변화에 따른 거동을 보여준다. 다음 절에서는 본 프레임워크의 downstream 활용 가능성에 관한 matched pipeline-variant 비교 결과를 제시한다.)[[not bad. this is connector paragraph. but feels too long for a connector paragraph. what do you think?]]

### 6.4 Downstream Pass-1-replacement 비교

(본 절은 direct collocation 대비 우월성 주장을 위한 자리가 아니다.)[[what '자리'? and unnessary line]] 두 pipeline (full two-pass DCM baseline과 Bézier-replaces-Pass-1 proposed)은 동일한 multi-phase LGL Pass 2 solver, dynamics 모형, tolerance, boundary-condition protocol을 공유하며, 유일한 차이는 warm-start trajectory와 phase 구조 결정의 출처이다. 따라서 본 비교는 "Bézier가 direct collocation보다 빠른가"의 비교가 아니라, "동일한 DCM pipeline 내부에서 Pass 1 단계를 Bézier SCP upstream으로 대체했을 때 최종 해와 end-to-end runtime이 어떻게 달라지는가"의 비교이다. 제안 pipeline이 작동할 수 있는 영역(regime)의 경계 또한 본 절의 일부로 보고한다.

**표 6 [T6]. Pass-1-replacement 비교 결과 (두 pipeline이 모두 수렴한 7개 circular 사례, boundary sweep run).**

| Case | $T_{\mathrm{normed}}$ | $h_0$ (km) | $\Delta a$ (km) | $\Delta i$ (deg) | Baseline (s) | Bézier (s) | Pass 2 (s) | Proposed total (s) | Speedup | $|\Delta \mathrm{cost}|$ | Peaks base / prop |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|:---:|
...
표 6은 동일한 Pass 2 protocol 하에서 두 pipeline이 모두 수렴한 7개 circular 사례를 정리한 것이다. 대상 사례는 $T_{\mathrm{normed}}$ 0.28에서 2.05까지 분포하여 sub-orbital 전이뿐 아니라 multi-revolution 전이(사례 20, 114)도 포함한다. 6개 사례에서 최종 비용은 baseline과 proposed가 $|\Delta \mathrm{cost}| < 10^{-9}$ 수준에서 일치하며, 이는 비용 스케일($\sim 10^{-3}$) 대비 기계 정밀도 수준의 보존이다. 사례 114는 예외로, Bézier upstream이 Pass 1보다 peak을 하나 더 검출하여(5개 vs 4개) Pass 2가 인접한 다른 국소해로 수렴하였고 비용 차이가 약 1.7%로 나타났다. (따라서 본 결과는 "Pass 1 단계의 완전한 drop-in 대체"를 지지하는 것이 아니라, "7개 사례 중 6개에서 동일 국소해에 수렴하고 1개 사례에서는 phase 구조 결정의 차이로 인해 결과가 달라진다"는 범위에서 해석되어야 한다.)[[reusing english sentence structure of 'not A but B']]
...
(본 절의 작동 영역은 두 가지 독립된 경계로 정의된다.)[[what is '본 절'?]] 첫째는 eccentricity 경계이다. 10개 사례의 eccentricity-boundary run($T_{\mathrm{normed}} \le 0.5$, $\max(e_0, e_f) \le 0.1$)에서 circular 사례 4/4는 Bézier upstream이 feasible하였으나, mildly elliptic 사례 6/6($e_0 = 0.050$, $e_f \approx 0.079$)은 모두 infeasible하였다. 곡선 내부가 모든 제어점이 출발 궤도 위에 놓여 있음에도 KOZ 내부로 파고들기 때문이다. 112개 boundary sweep 사례에서도 Bézier-feasible 사례 34개는 전부 $e_0 = e_f = 0$인 circular 사례였으므로, 본 데이터셋 안에서 eccentricity 경계는 sharp하다. 둘째는 전이 시간 경계이다. Boundary sweep의 $T_{\mathrm{normed}}$ 구간별 양측-수렴 비율은 $T \le 0.5$에서 4/4, $0.5 < T \le 1.0$에서 1/3, $1.0 < T \le 2.0$에서 1/14, $T > 2.0$에서 1/91이다. 즉, Bézier upstream은 multi-revolution 사례에서도 드물게($15/91$) feasible할 수 있으나, Pass 2가 해당 warm-start로부터 수렴하는 비율이 급격히 낮아진다. 따라서 proposed pipeline은 eccentricity 축에서는 circular 제약이 sharp하고, 전이 시간 축에서는 downstream Pass 2의 수렴이 soft한 병목으로 작용한다.

본 절의 결과는 다음 두 가지 해석 경계 안에서 읽혀야 한다. 첫째, 표 6은 matched Pass 2 protocol 하의 작동 영역 안에서 성립하는 pipeline-variant 초기화 claim을 지지하며, direct collocation 대비 method-class 우월성 주장이 아니다. Proposed pipeline 자체가 하나의 DCM pipeline이며, Pass 1 단계의 구현 방식만 baseline과 다르다. 둘째, 작동 영역이 좁고(converged 112개 중 양측-수렴 7개) sample 수가 제한적이므로, 측정된 speedup 분포는 Bézier warm-start로부터 Pass 2가 수렴하지 못하는 사례의 모집단으로 외삽해서는 안 된다. 정직한 읽기는, 서술된 가정 하에서 Bézier upstream이 Pass 1 단계의 사용 가능한 대체이며, 일반적인 downstream solver 가속기가 아니라는 것이다.

---

## 7. 한계 및 해석 범위

(본 연구의 결과는 다음과 같은 범위 안에서 해석해야 한다.)[[unnatural]]
...
여섯째, §6.4의 Pass-1-replacement 결과는 두 가지 독립된 작동 영역 경계 안에서만 성립한다. (1) Eccentricity 경계: 테스트된 데이터셋 안에서 모든 nonzero $e_0$ 또는 $e_f$ 사례는 Bézier upstream이 infeasible하였다. (2) 전이 시간 경계: $T_{\mathrm{normed}} > 1$ 구간에서 Bézier upstream이 feasible하더라도 downstream Pass 2가 해당 warm-start로부터 수렴하는 비율은 현저히 낮다. 또한 sample 수가 작으므로(양측-수렴 7개 사례) 측정된 speedup 분포를 이 영역 밖으로 외삽해서는 안 된다. 사례 114에서 관찰된 peak 수 차이는 Bézier upstream이 Pass 1의 phase 구조 결정을 항상 동일하게 재현하지 않음을 보여주며, 이 점은 drop-in 대체가 아닌 pipeline-variant 비교로 framing되어야 함을 의미한다.
