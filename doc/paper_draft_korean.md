...
본 절에서는 제안 기법을 세 가지 맥락에서 살펴본다. 첫째는 direct transcription 및 direct collocation 계열의 궤적 최적화 방법, 둘째는 obstacle avoidance를 위한 컨벡스화 또는 (보수적 안전 처리)[[awkard word choice]] 방식, 셋째는 후속 최적화를 위한 초기화와 warm start 생성 방법이다.
...
각 $\mathbf{q}^{(s)}_k$는 원래 제어점의 선형결합이므로 이 제약식은 결정 변수 $\mathbf{x}$에 대해 선형이다. 반공간은 각 SCP 반복에서 현재 해를 기준으로 다시 구성되며, 따라서 현재 해 주변에서 작동하는 보수적이고 국소적인 회피 제약조건으로 이해할 수 있다. 어떤 분할구간의 모든 제어점이 $\mathcal{H}^{(s)}$ 안에 있으면, Bézier 곡선의 볼록 껍질 성질에 의해 그 분할구간의 곡선 전체도 $\mathcal{H}^{(s)}$ 안에 놓이고, 따라서 $\mathcal{K}$ 바깥에 놓인다 [1].
