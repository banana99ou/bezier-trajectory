...
의 정확한 이차형식으로 정리된다. 이로써 곡선의 매끄러움을 재는 목적항이 볼록 이차함수로 표현되어, 3절의 convex QP에 그대로 사용된다.
...
본 연구에서는 (affine하게 선형화한 중력장)[[이게 뭔지 설명 부터.]] 아래에서 제어 가속도 u(t)를 근사적으로 반영하는 대리 목적함수를 사용한다. 우선
