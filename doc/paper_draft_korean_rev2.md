...
**그림 1 [F1].** 하나의 분할구간에 대한 구형 KOZ 선형화 개념도. 분할구간 제어점의 중심점에서 구성한 지지 반공간 제약을 해당 분할구간의 모든 제어점에 부과함으로써 구형 KOZ를 보수적으로 배제한다. [[그림의 상세내용에 대한 서술이 미비함.]]
...
여기서 $H^{(k)}$와 $\mathbf{f}^{(k)}$는 3.2절의 목적함수 $J^{(k)}$를 전개한 것으로, $M_i^{(k)} = A_i - B_i^{(k)}$라 하면

$$
H^{(k)} = 2\left(\frac{1}{T^4}\,\tilde G_N \otimes I_3 + \sum_i w_i\,(M_i^{(k)})^{\mathsf{T}} M_i^{(k)}\right), \qquad
\mathbf{f}^{(k)} = -2\sum_i w_i\,(M_i^{(k)})^{\mathsf{T}}\mathbf{c}_i^{(k)}
$$

이다. $\mathbf{f}^{(k)}$는 중력 선형화의 상수항 $\mathbf{c}_i^{(k)}$에서 비롯되는 1차 항이며, 해에 영향을 주지 않는 상수항은 생략하였다.

여유 변수(slack variable) $\mathbf{s}$는 선형화된 KOZ 제약의 실현 가능성을 보완해주는 항으로, SCvx의 virtual control [6, 7]에 해당한다. 기준점이 KOZ 안쪽에 놓이는 초기 단계에서는 선형화된 제약이 그 자체로 실현 불가능할 수 있으므로, 여유 변수로 이를 흡수하되 큰 페널티 계수 $w_s$를 두어 수렴한 해에서는 $\mathbf{s} = \mathbf{0}$이 되도록 한다. 신뢰 구간 제약 $\|\mathbf{x} - \mathbf{x}^{(k)}\|_\infty \le r_k$은 선형화가 유효한 범위 밖으로 벗어나는 해를 막으며, 신뢰 구간의 크기 $r_k$는 아래의 기준과 연동하여 반복마다 조절된다.

하위 문제의 해 $\hat{\mathbf{x}}$를 새로운 기준점으로 삼을지는, 표준적인 SCvx에서는 목적함수와 제약 조건 위반을 결합한 확대 목적함수(merit function)로 평가한다 [6, 7]. 그러나 본 문제에서는 목적항과 KOZ 위반의 척도 차이가 매우 커서 하나의 가중치로 두 항의 균형을 맞추기 어렵다. 이 때문에 평가를 두 단계로 나눈다. 곡선이 실제 KOZ를 위반하는 동안에는 실제 위반이 줄어드는 경우에만 $\hat{\mathbf{x}}$를 새로운 기준점으로 간주한다(feasibility restoration). 이 단계에서 목적함수는 고려하지 않는다. KOZ를 통과하는 곡선들 사이에서 비용을 비교하는 것은 의미가 없기 때문이다. 곡선이 실현 가능해진 뒤에는, 하위 문제가 계산한 예측 감소량에 대한, 중력을 비선형 그대로 두고 계산한 실제 감소량의 비

$$
\rho_k = \frac{J(\mathbf{x}^{(k)}) - J(\hat{\mathbf{x}})}{J^{(k)}(\mathbf{x}^{(k)}) - J^{(k)}(\hat{\mathbf{x}})}
$$

를 기준으로 삼는다. $\rho_k$가 1에 가깝다는 것은 선형화 모형이 실제 목적 변화를 정확히 예측했다는 뜻이다. 따라서 $\rho_k$가 기준값 $\eta$를 넘으면 $\hat{\mathbf{x}}$를 새로운 기준점으로 간주하고, 1에 가까우면 신뢰 구간의 크기를 늘려 더 큰 이동을 허용한다. 반대로 $\rho_k \le \eta$이면 $\hat{\mathbf{x}}$를 버리고 신뢰 구간의 크기를 줄여 같은 기준점에서 다시 풀며, 실제 KOZ를 다시 위반하는 해는 $\rho_k$와 무관하게 버린다.

수렴 조건은 새로운 기준점에서 목적함수의 상대 변화가 허용오차보다 작아지는 것으로 둔다. 지지 반공간의 법선이 반복마다 다시 계산되어 제어점이 제약면을 따라 미세하게 움직일 수 있으므로, 제어점 변화의 크기는 수렴 조건으로 쓰지 않는다. 신뢰 구간의 크기가 하한에 도달한 경우에도 종료한다. 이로써 제안 기법은 매 반복에서 볼록 QP 하나를 풀며 실현 가능성과 목적 개선을 차례로 확보하는, 신뢰 구간 기반의 국소 최적화 알고리즘으로 이해할 수 있다.
...
> 6. &nbsp;&nbsp; 곡선이 실제 KOZ를 위반하면: 위반이 줄어든 경우에만 $\hat{\mathbf{x}}$를 새로운 기준점으로 간주하고 신뢰 구간의 크기를 늘리며, 그렇지 않으면 줄인다
> 7. &nbsp;&nbsp; 그 외에는: $\rho_k$를 계산하여, $\rho_k > \eta$이고 KOZ를 다시 위반하지 않으면 새로운 기준점으로 간주하고 $\rho_k$가 1에 가까우면 신뢰 구간의 크기를 늘린다; 그렇지 않으면 $\hat{\mathbf{x}}$를 버리고 줄인다
> 8. &nbsp;&nbsp; 목적함수의 상대 변화가 허용오차보다 작거나 신뢰 구간의 크기가 하한에 도달하면 종료한다

신뢰 구간의 초기 크기와 조절 계수, 기준값 $\eta$, 페널티 계수 $w_s$의 값은 4.1절에 제시한다.

<a id="fig-scp-pipeline"></a>
![그림 2. 제어점 공간에서의 SCvx 반복](../figures/f2_scp_pipeline.png)
**그림 2 [F2].** 제안 SCvx 반복의 제어점 공간 구현. 매 반복에서 지지 반공간과 중력 선형화를 다시 만들고 볼록 QP를 풀어 기준점을 옮겨 가는 구조를 보여준다.
...
먼저 제안 기법이 대상 궤도전이 문제에서 실제로 실현 가능한 궤적을 생성하는지 확인한다. 대표 궤적의 예시는 [그림 3](#fig-trajectory)에 제시하였고, 정량 결과는 표 2에 요약하였다.

<a id="fig-trajectory"></a>
![그림 3. 대표 궤도전이 궤적 예시](../figures/f3_representative_settings.png)
**그림 3 [F3].** Bézier 차수별 궤도전이 궤적.
...
![그림 4. 분할 수에 따른 계산 시간 및 결과 추세](../figures/f4_subdivision_tradeoff_N7.png)
**그림 4 [F4].** 120 deg 위상차 시나리오($N=7$)에서 분할 수에 따른 안전 여유·제어 비용·계산 시간의 변화.[[n_seg==2 infeasible case임을 별도로 표시]]
...
**그림 5 [F5].** 120 deg 위상차 시나리오에서 $N=6,7,8$ 차수에 대한 제어 비용(좌)과 계산 시간(우)의 추세.
...
![그림 6. 1단계 대체의 사례별 전체 계산 시간 비 및 구성](../figures/f6_downstream_speedup.png)
**그림 6 [F6].** 1단계 대체가 전체 계산 시간에 미치는 영향을 보이기 위한 사례별 비교. 좌측은 전체 계산 시간 비, 우측은 단계별 계산 시간 구성이다.
