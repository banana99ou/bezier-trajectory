...
로 정의한다. 여기서 $\mathbf{g}$는 궤도 중력 모델로, 현재 실험에서는 이체 문제(two-body problem) 항과 J2 섭동(perturbation) 항을 포함한다.
...
**그림 4 [F4].** 120 deg 위상차 시나리오에서 $N=7$ 기준 분할 수에 따른 계산 시간 및 결과 변화. $n_{\mathrm{seg}}=2$는 실행 불가능(음의 안전 여유)하며, $n_{\mathrm{seg}} \ge 4$에서 안전 여유는 약 144 km에서 약 1 km로, 제어 비용은 약 9,288 m/s에서 약 6,292 m/s로 단조 감소한다. 계산 시간은 약 34 s에서 약 145 s로 증가하여, 분할 정밀화에 따른 보수성 감소-계산 비용 상충 관계를 보여준다.[[n_seg==2 infeasible case임을 별도로 표시]]
...
**그림 5 [F5].** 120 deg 위상차 시나리오에서 $N=6,7,8$ 차수에 대한 성능 추세. 좌측 패널은 분할 수에 따른 제어 비용 변화를, 우측 패널은 같은 설정에서의 계산 시간을 보여준다. 세 차수 모두 $n_{\mathrm{seg}} \ge 8$에서 유사한 제어 비용 수준에 수렴하며, $N=8$이 전반적으로 가장 낮은 제어 비용을 보이나 계산 시간은 가장 크다. 차수 선택은 전면적 우월성이 아닌 제어 비용-계산 시간 상충 관계로 해석해야 한다.
...
![그림 6. 1단계 대체의 사례별 속도 향상 및 계산 시간 구성](../figures/f6_downstream_speedup.png)
