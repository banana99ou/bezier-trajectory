...
| `C9` | The Bézier upstream can replace Pass 1 of a two-pass direct-collocation pipeline, within the scoped regime of circular transfers in which both pipelines converge; on the tested set the final cost is preserved on 6 of 7 cases and end-to-end runtime is reduced on 4 of 7. | Matched comparison between the full two-pass DCM pipeline (Pass 1 H-S → peak detect → Pass 2 LGL) and a variant in which the Bézier SCP upstream replaces Pass 1 while Pass 2, dynamics, tolerances, and boundary-condition protocol remain identical; report convergence, cost parity, runtime composition, and both regime boundaries (eccentricity and transfer time). | `present` | Owned by `doc/dcm_downstream_pack.md`; backed by `artifacts/paper_artifacts/t6_downstream_comparison.csv` (112 cases, 7 both-converged, `T_normed` from `0.28` to `2.05`) and `figures/f6_downstream_speedup.png`. On 6 of 7 cases `|cost_delta| < 1e-9`; on case 114 the Bézier upstream finds one more peak than Pass 1 and Pass 2 converges to a nearby optimum with a `1.7%` cost delta. Median speedup `1.22×` (min `0.51×`, max `2.47×`). Regime boundaries documented: eccentricity boundary from a 10-case `max_ecc ≤ 0.1` run, transfer-time boundary from the 112-case boundary sweep. | Keep wording tied to the Pass-1-replacement framing, the scoped regime, and the `6 / 7` equivalence rate; do not promote this to a method-class superiority claim over direct collocation, and flag the case-114 phase-structure mismatch as a material caveat rather than hiding it. |
...
- `C9` (scoped): The Bézier upstream can replace Pass 1 of a two-pass DCM pipeline within the scoped circular-short-transfer regime, preserving final cost and reducing end-to-end runtime under matched Pass 2.
...
2. ~~A downstream comparison: naive direct collocation versus direct collocation initialized by the Bézier-based warm start.~~ DONE via reframing: the Pass-1-replacement comparison in `doc/dcm_downstream_pack.md` supplies the C9 evidence. Original naive-vs-warm-started framing was closed by Finding 2 and replaced with pipeline-variant framing; T6 and F6 are data-ready.
