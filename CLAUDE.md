...
- B8 rename the objective to what it measures, and add the velocity/acceleration rows as control-point operators (formulation decisions 1–3; the derivation is done, in PAPER_1)
...
- B12 occlusion constraint builder, moving non-straight occluder as a chain of straight tubes. **Gated** on the per-piece space-time convexity check — do not write it before that check runs (PAPER_1 §"Occluder geometry")
