...
- **G2 — FIXED 2026-08-17, and the key widened 2026-08-26.** One plane per **(segment, obstacle, connected component of the clipped KOZ volume)**, aimed at the segment centroid, plus a rotation term in the subproblem. It was one plane per (segment, obstacle) until the wall moved onto the clipped volume; grouping rows without the component index folds two genuinely different walls together. The dense subdivision matrix was never part of this defect — sparsifying it would *break* the guarantee.
...
- **A clip ball CAN catch the same tube twice, and `station_gate` does it.** Time monotonicity keeps the lifted tube from touching itself, so it never severs in the self-intersection sense — but an obstacle that sweeps toward the corridor and back presents two separated passes to one clip ball, which is two components and two walls. Measured 2026-08-26: 32 of 144 (segment, obstacle, trust) probes on `station_gate`, none on `curve`, `original`, `wall`, `fence3d` or `door3d`. **Do not repeat the claim that no scenario exercises the multi-wall path.**
