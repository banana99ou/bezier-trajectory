...
2. A figure comes from a run where elastic slack hid a constraint violation. (Closed 2026-08-20 — B7's `figure_grade` gate, and `tools/make_paper_figure.py` refuses to draw a run that fails it. **Extended 2026-08-31 with a sixth condition: zero `koz_unsound_clips`** — what the certificate COVERS, PAPER_1 statement (7) at the returned iterate. The Rust core counted it from the day the clip landed, `optimize_scenario` never put it in the result row and the predicate never read it, so on this condition the gate could not fail. **Measured consequence: 26 of the 28 registered configurations were being graded figure-grade on a certificate that covers less than the obstacle** (1 to 197 uncovered WALLS — the count is per wall, one per local approach, not per (segment, obstacle) pair; only `curve` N10_seg8 and `loiter` N8_seg16 were clean). `sound_clip=True` zeroes every one of them; figure-grade goes 2 of 28 → 23 of 28. The floor is free on `original`, `diverse`, `fence3d`, `door3d` and `loiter`, costs `wall` real clearance, and costs `curve` N8_seg16 its certificate. **It is NOT the default** — see README §Measurements, which is the evidence for whether it should be.)
...
- ~~B12 line-of-sight occlusion~~ **DONE 2026-08-20, REBUILT 2026-08-30** — `ed8c50a`+`dd51d3e`+`99a6714`, then `46a15f7`. **There is no occlusion builder, and there are not two keep-out zones.** An obstacle's zone and the shadow it casts are one set at two stretch factors, so the centreline generalizes to a **center surface**: the shadow of the centreline with the ground station as a point light source. `u = 1` is the body — the obstacle's radius never changes, the umbra widens, because a point source at finite distance casts constant *angular* thickness. For a fixed `u` the stretch is affine, so the surface is again a Bezier curve and subdivision, hodographs and the De Casteljau support bounds carry over unchanged. **No station means `u = 1` identically and every formula reduces to the plain lifted tube** — that reduction is the regression the rewrite stands on, witnessed by `tests/unit/test_spacetime_koz_geometry.py`. Derivation and the falsification test live in the `rust_optimizer/core/src/spacetime_generator.rs` module docstring and `tests/unit/test_center_surface_geometry.py`
  ...
  - **`station_fence` was REMOVED 2026-08-30 (`paper/journal-1`).** Its measured numbers were all taken under the retired builder, and its own test file asserted the retired design — that a curved occluder must be a chain of *straight* pieces, because a capsule around a curved centreline is not convex. The center surface handles the curved centreline directly, so that file was a test of a decision the code no longer makes. `loiter` is the occlusion scenario now and inherited the live coverage. The 2D-is-genuinely-infeasible finding was geometry, not builder, and stands
...
- **`obstacle_pos0` / `obstacle_vel` are not parameters of anything.** `0918df5` moved the Rust API to lifted control points and did not update the tests. 75 of 338 tests were red at that commit and stayed red; 13 of them were `tests/unit/test_spacetime_koz_geometry.py`, which CLAUDE.md lists as the Rust builder's coverage — so **the Rust builder had no live coverage at all** between `0918df5` and 2026-08-26. Repaired for that file; **47 are still red on `paper/journal-1` (47 failed / 296 passed / 1 skipped, measured 2026-08-30)**, down from 52 only because removing `station_fence` took its 6-test file with it. **They fail at the CALL, not on an assertion** — `TypeError: unexpected keyword 'obstacle_pos0'` in 0.09 s — so these files are DARK, not stale: not one of their assertions has executed since `0918df5`. Measured breakdown of the 47: 23 `obstacle_pos0` TypeError, 16 `ValueError` legacy-window, 6 `KeyError: 'pos0'`, 1 behaviour drift (`test_degree_two_bezier_rejected` no longer raises), and 1 real unexplained move (`test_speed_cap_diagnostics.py:154`, arrival 15.56 where the docstring records 19.78). The dark files carry the paper's own claims: `test_objective_matches_rust.py` (15, item B8), `test_speed_cap_and_time_penalty.py` (7, B9/B10), `test_los_margin.py` (5, the independent line-of-sight check the figure quotes) and `test_clearance_sampling.py` (5, **how `min_clearance` is computed — the source of every clearance in README §Measurements**). `test_occlusion_geometry.py` is no longer among them because `46a15f7` deleted it — every one of its assertions was about the retired builder's objects. Convert `pos0`/`vel` obstacles with `spacetime_bezier.geometry.obstacle_array_bundle`; a legacy obstacle with no window needs one, because the active window is now intrinsic to the control points.
...
paper figure is the priced run. `station_fence` was removed from the registry 2026-08-30 and its
two rows went with it.
